...
static void createProgramGroups(OptixInternalState &state)
{
    ...
    hitgroupProgramGroupDesc.hitgroup.moduleAH = state.module;
...
static void createShaderBindingTable(
    ...
    for (const auto &geometryResult : sceneState.geometries) {
        for (const auto &record : geometryResult.hostSBTRecords) {
            ...
            hitgroupSbt.data.texCoords = reinterpret_cast<float *>(record.d_texCoords);
            hitgroupSbt.data.texCoordsIndices = reinterpret_cast<int *>(record.d_texCoordsIndices);
            #if TEX_MOANA
            ...
            #else
            hitgroupSbt.data.baseColor = Materials::materialValueForMesh(record.materialName).first;
            hitgroupSbt.data.bsdfType = Materials::materialValueForMesh(record.materialName).second;
            #endif
